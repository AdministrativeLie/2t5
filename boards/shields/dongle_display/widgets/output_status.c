...
int zmk_widget_output_status_init(struct zmk_widget_output_status *widget, lv_obj_t *parent) {
    ...
    lv_obj_align(usb, LV_ALIGN_LEFT_MID, 5, 0);  // Align Left
    lv_img_set_src(usb, &sym_usb);

    lv_obj_t *usb_hid_status = lv_img_create(widget->obj);
    lv_obj_align_to(usb_hid_status, usb, LV_ALIGN_OUT_RIGHT_MID, 5, 0);

    lv_obj_t *bt = lv_img_create(widget->obj);
    lv_obj_align_to(bt, usb_hid_status, LV_ALIGN_OUT_RIGHT_MID, 5, 0);

    lv_obj_t *bt_number = lv_img_create(widget->obj);
    lv_obj_align_to(bt_number, bt, LV_ALIGN_OUT_RIGHT_MID, 5, 0);

    lv_obj_t *bt_status = lv_img_create(widget->obj);
    lv_obj_align_to(bt_status, bt, LV_ALIGN_OUT_RIGHT_MID, 5, 0);
